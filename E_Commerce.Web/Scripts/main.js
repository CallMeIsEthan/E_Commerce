...
*
* Modified: Removed Drift.js zoom functionality - using GLightbox for click-to-view instead
* Version: 2.0 - No Drift dependencies
*/
...
(function () {
    ...
    function toggleScrolled() {
        ...
        // Luôn thêm class scrolled nếu header có sticky-top
        if (selectHeader.classList.contains('sticky-top') || selectHeader.classList.contains('fixed-top') || selectHeader.classList.contains('scroll-up-sticky')) {
    ...
    function productDetailFeatures() {
        // Check if we're on a product details page before initializing
        const mainImage = document.getElementById('main-product-image');
        if (!mainImage) {
            return; // Not on product details page, skip initialization
        }
        ...
        let navigationHandlers = { prev: null, next: null };
        ...
        function initImageNavigation() {
            ...
            if (navigationHandlers.prev) {
                prevButton.removeEventListener('click', navigationHandlers.prev);
            }
            if (navigationHandlers.next) {
                nextButton.removeEventListener('click', navigationHandlers.next);
            }

            // Function to get current visible thumbnails
            function getVisibleThumbnails() {
                const allThumbnails = Array.from(document.querySelectorAll('.thumbnail-item'));
                // Filter out hidden thumbnails
                return allThumbnails.filter(thumb => {
                    const style = window.getComputedStyle(thumb);
                    return style.display !== 'none' && style.visibility !== 'hidden' && thumb.offsetParent !== null;
                });
            }
            ...
            function navigateImage(direction) {
                ...
                // Update active state - ensure only one is active
                thumbnails.forEach(item => item.classList.remove('active'));
            ...
            navigationHandlers.prev = function (e) {
                e.preventDefault();
                e.stopPropagation();
                navigateImage('prev');
            };

            navigationHandlers.next = function (e) {
                e.preventDefault();
                e.stopPropagation();
                navigateImage('next');
            };
            ...
            // Add event listeners
            prevButton.addEventListener('click', navigationHandlers.prev);
            nextButton.addEventListener('click', navigationHandlers.next);
        }
        ...
        // Expose initImageNavigation for re-initialization when thumbnails change
        if (typeof window.productDetailFeatures === 'undefined') {
            window.productDetailFeatures = {};
        }
        window.productDetailFeatures.initImageNavigation = initImageNavigation;
    }

    // Only run productDetailFeatures if on a product details page
    // This prevents errors if the page doesn't have product details elements
    // Wrap in try-catch to prevent errors if Drift is referenced but not loaded
    try {
        if (document.getElementById('main-product-image') || document.querySelector('.product-details')) {
            productDetailFeatures();
        }
    } catch (error) {
        // Silently ignore errors related to removed Drift functionality
        if (error.message && error.message.includes('Drift')) {
            // Ignore Drift errors silently
        } else {
            console.warn('Product detail features initialization error:', error);
        }
    }
    ...
    function initOnePageCheckout() {
        ...
        // REMOVED: Template's default form handler - we use custom AJAX checkout
        // Skip if form has id="checkoutForm" (our custom checkout with AJAX)
        const checkoutForm = document.querySelector('.checkout-form');
        if (checkoutForm && checkoutForm.id === 'checkoutForm') {
            console.log('Skipping template checkout form handler - using custom AJAX');
            // Do nothing - let our custom AJAX handler in Checkout.cshtml handle everything
            return;
        }
    }
